...
export class RoomResolverBase {
  ...
  @graphql.ResolveField(() => [Desk], { name: "desks" })
  async findDesks(
    @graphql.Parent() parent: Room,
    @graphql.Args() args: DeskFindManyArgs
  ): Promise<Desk[]> {
    const results = await this.service.findDesks(parent.id, args);
  ...
  async getOffice(@graphql.Parent() parent: Room): Promise<Office | null> {
